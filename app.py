...
def create_pdf(results):
    ...
    pdf.cell(0, 5, f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} UTC", ln=True, align='C')
    pdf.ln(8)
    ...
    for idx, res in enumerate(results):
        ...
        pdf.cell(0, 8, f"#{idx+1:02d}  {res['filename']}", ln=True)
        ...
        pdf.cell(0, 8, f"{res['label'].upper()}", ln=True)
        pdf.set_x(75)
        pdf.set_font("Arial", '', 11)
        pdf.set_text_color(60, 60, 60)
        pdf.cell(0, 7, f"Confidence: {res['confidence']:.2f}%", ln=True)
        pdf.set_y(y + 60)
    ...
    pdf.multi_cell(0, 4, "DISCLAIMER: This report is generated by a Deep Learning model for research purposes only. Final diagnosis must be conducted by a qualified medical professional.")
